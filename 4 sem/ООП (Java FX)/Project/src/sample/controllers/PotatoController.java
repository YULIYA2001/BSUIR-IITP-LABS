...
public class PotatoController {
        ...
        toCartButton.setOnAction(event -> {
            ...
            try {
                ...
                while (in.hasNextLine()) {
                    jsonPotato = in.nextLine();
                    if (jsonPotato.contains("POTATO"))
                        break;
                }
                in.close();
            }
            ...
            String jsonNewPotato = null;
            // если в корзине есть товар, добавляем к нему новое количество и пересчет суммы
            if (jsonPotato != null && jsonPotato.contains("POTATO")) {
                if (amountSlider.getValue() != 0.0) {
                    Cart potato = MyJSON.fromJSON(jsonPotato, Cart.class);
                    double sumAmount = potato.getAmount() + amountSlider.getValue();
                    Cart newPotato = new Cart(potato.getProduct(), sumAmount);
                    potato = null;    // для сборщика мусора
                    jsonNewPotato = MyJSON.toJSON(newPotato);
                }
            }
            // если товара нет в корзине, создаем его
                ...
                if (amountSlider.getValue() != 0.0) {
                    ...
                    jsonNewPotato = MyJSON.toJSON(cart);
                }
            }
            ...
            if (jsonNewPotato != null && jsonPotato == null) {
                try (FileOutputStream fos = new FileOutputStream(Main.passCartsFolder
                        + "\\" + Main.currentClient.getLogin() + ".txt", true)) {
                    byte[] buffer = (jsonNewPotato + "\n").getBytes();
                    fos.write(buffer, 0, buffer.length);
                    System.out.println("The file " + Main.currentClient.getLogin() +
                            ".txt in Carts folder was changed from PotatoController 1");
                } catch (IOException ex) {
                    System.out.println(ex.getMessage());
                }
                showSuccessOperationAdd();
            }
            else if (jsonNewPotato != null && jsonPotato != null){

                writeNewDeleteOld(jsonNewPotato, jsonPotato);
                showSuccessOperationAdd();
            }
        });
    }
    ...
    private void writeNewDeleteOld(String jsonNew, String json) {
        ...
        try {
            ...
            out.close();
            System.out.println("The file " + Main.currentClient.getLogin() +
                    ".txt in Carts folder was changed from PotatoController 2");
        } catch (IOException ex) {
    ...
    private void showSuccessOperationAdd() {
        ...
        try {
            ...
            e.printStackTrace();
            System.out.print("WRONG IN success_add_to_cart.fxml from PotatoController");
        }
        Parent root = loader.getRoot();
    ...
    private Product loadInformation() {
        ...
        for (int i = 0; i < len; i++){
            if (Company.getInstance().getProducts().get(i).getName() == FruitVegetable.POTATO){
