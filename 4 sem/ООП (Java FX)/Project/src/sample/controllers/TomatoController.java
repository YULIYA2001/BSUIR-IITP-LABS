...
public class TomatoController {
        ...
        toCartButton.setOnAction(event -> {
            ...
            try {
                ...
                while (in.hasNextLine()) {
                    jsonTomato = in.nextLine();
                    if (jsonTomato.contains("TOMATO"))
                        break;
                }
                in.close();
            }
            ...
            String jsonNewTomato = null;
            // если в корзине есть товар, добавляем к нему новое количество и пересчет суммы
            if (jsonTomato != null && jsonTomato.contains("TOMATO")) {
                if (amountSlider.getValue() != 0.0) {
                    Cart tomato = MyJSON.fromJSON(jsonTomato, Cart.class);
                    double sumAmount = tomato.getAmount() + amountSlider.getValue();
                    Cart newTomato = new Cart(tomato.getProduct(), sumAmount);
                    tomato = null;    // для сборщика мусора
                    jsonNewTomato = MyJSON.toJSON(newTomato);
                }
            }
            // если товара нет в корзине, создаем его
                ...
                if (amountSlider.getValue() != 0.0) {
                    ...
                    jsonNewTomato = MyJSON.toJSON(cart);
                }
            }
            ...
            if (jsonNewTomato != null && jsonTomato == null) {
                try (FileOutputStream fos = new FileOutputStream(Main.passCartsFolder
                        + "\\" + Main.currentClient.getLogin() + ".txt", true)) {
                    byte[] buffer = (jsonNewTomato + "\n").getBytes();
                    fos.write(buffer, 0, buffer.length);
                    System.out.println("The file " + Main.currentClient.getLogin() +
                            ".txt in Carts folder was changed from TomatoController 1");
                } catch (IOException ex) {
                    System.out.println(ex.getMessage());
                }
                showSuccessOperationAdd();
            }
            else if (jsonNewTomato != null && jsonTomato != null){

                writeNewDeleteOld(jsonNewTomato, jsonTomato);
                showSuccessOperationAdd();
            }
        });
    }
    ...
    private void writeNewDeleteOld(String jsonNew, String json) {
        ...
        try {
            ...
            out.close();
            System.out.println("The file " + Main.currentClient.getLogin() +
                    ".txt in Carts folder was changed from TomatoController 2");
        } catch (IOException ex) {
    ...
    private void showSuccessOperationAdd() {
        ...
        try {
            ...
            e.printStackTrace();
            System.out.print("WRONG IN success_add_to_cart.fxml from TomatoController");
        }
        Parent root = loader.getRoot();
    ...
    private Product loadInformation() {
        ...
        for (int i = 0; i < len; i++){
            if (Company.getInstance().getProducts().get(i).getName() == FruitVegetable.TOMATO){
