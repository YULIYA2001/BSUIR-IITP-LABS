...
public class CucumberController {
    ...
    void initialize() {
        ...
        toCartButton.setOnAction(event -> {
            ...
            String jsonCucumber = null;
            // ищем в корзине клиента заказ на огурцы
            try {
                Scanner in = new Scanner(new File(Main.passCartsFolder
                        + "\\" + Main.currentClient.getLogin() + ".txt"), "UTF-8");
                while (in.hasNextLine()) {
                    jsonCucumber = in.nextLine();
                    if (jsonCucumber.contains("CUCUMBER"))
                        break;
                }
                in.close();
            }
            ...
            String jsonNewCucumber = null;
            // если в корзине есть товар, добавляем к нему новое количество и пересчет суммы
            if (jsonCucumber != null && jsonCucumber.contains("CUCUMBER")) {
                if (amountSlider.getValue() != 0.0) {
                    Cart cucumber = MyJSON.fromJSON(jsonCucumber, Cart.class);
                    double sumAmount = cucumber.getAmount() + amountSlider.getValue();
                    Cart newCucumber = new Cart(cucumber.getProduct(), sumAmount);
                    cucumber = null;    // для сборщика мусора
                    jsonNewCucumber = MyJSON.toJSON(newCucumber);
                }
            }
            // если товара нет в корзине, создаем его
                ...
                if (amountSlider.getValue() != 0.0) {
                    ...
                    jsonNewCucumber = MyJSON.toJSON(cart);
                }
            }
            ...
            if (jsonNewCucumber != null && jsonCucumber == null) {
                try (FileOutputStream fos = new FileOutputStream(Main.passCartsFolder
                        + "\\" + Main.currentClient.getLogin() + ".txt", true)) {
                        byte[] buffer = (jsonNewCucumber + "\n").getBytes();
                        fos.write(buffer, 0, buffer.length);
                        System.out.println("The file " + Main.currentClient.getLogin() +
                                ".txt in Carts folder was changed from CucumberController 1");
                } catch (IOException ex) {
                    System.out.println(ex.getMessage());
                }
                showSuccessOperationAdd();
            }
            else if (jsonNewCucumber != null && jsonCucumber != null){

                writeNewDeleteOld(jsonNewCucumber, jsonCucumber);
                showSuccessOperationAdd();
            }
        });
    }
    ...
    private void writeNewDeleteOld(String jsonNew, String json) {
        ...
        try {
            ...
            out.close();
            System.out.println("The file " + Main.currentClient.getLogin() +
                    ".txt in Carts folder was changed from CucumberController 2");
        } catch (IOException ex) {
    ...
    private void showSuccessOperationAdd() {
        ...
        try {
            ...
            e.printStackTrace();
            System.out.print("WRONG IN success_add_to_cart.fxml from CucumberController");
        }
        Parent root = loader.getRoot();
    ...
    private Product loadInformation() {
        ...
        for (int i = 0; i < len; i++){
            if (Company.getInstance().getProducts().get(i).getName() == FruitVegetable.CUCUMBER){
