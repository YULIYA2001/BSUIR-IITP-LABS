...
public class PearController {
    ...
    void initialize() {
        ...
        amountSlider.valueProperty().addListener((changed, oldValue, newValue) -> weightLabel.setText("" + newValue));
        ...
        toCartButton.setOnAction(event -> {
            ...
            try {
                ...
                while (in.hasNextLine()) {
                    jsonPear = in.nextLine();
                    if (jsonPear.contains("PEAR"))
                        break;
                }
                in.close();
            }
            ...
            String jsonNewPear = null;
            // если в корзине есть товар, добавляем к нему новое количество и пересчет суммы
            if (jsonPear != null && jsonPear.contains("PEAR")) {
                if (amountSlider.getValue() != 0.0) {
                    Cart pear = MyJSON.fromJSON(jsonPear, Cart.class);
                    double sumAmount = pear.getAmount() + amountSlider.getValue();
                    Cart newPear = new Cart(pear.getProduct(), sumAmount);
                    pear = null;    // для сборщика мусора
                    jsonNewPear = MyJSON.toJSON(newPear);
                }
            }
            // если товара нет в корзине, создаем его
                ...
                if (amountSlider.getValue() != 0.0) {
                    ...
                    jsonNewPear = MyJSON.toJSON(cart);
                }
            }
    ...
    private void writeNewDeleteOld(String jsonNew, String json) {
        ...
        try {
            ...
            out.close();
            System.out.println("The file " + Main.currentClient.getLogin() +
                    ".txt in Carts folder was changed from PearController 2");
        } catch (IOException ex) {
    ...
    private void showSuccessOperationAdd() {
        ...
        try {
            ...
            e.printStackTrace();
            System.out.print("WRONG IN success_add_to_cart.fxml from PearController");
        }
        Parent root = loader.getRoot();
    ...
    private Product loadInformation() {
        ...
        for (int i = 0; i < len; i++){
            if (Company.getInstance().getProducts().get(i).getName() == FruitVegetable.PEAR){
