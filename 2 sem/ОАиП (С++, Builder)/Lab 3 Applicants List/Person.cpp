...
void List :: Add(String surname, String name, String patronymic, AnsiString address,
...
void List :: SearchByCity(TMemo *Memo1, AnsiString city)
{
	Person *person = head;
	AnsiString addressCity;
	int check, i;
	while (person != NULL)
	{	// если введенный город(подстрока) найден в адресе(строка)
		// начиная с 1 позиции(с начала)
		if (person->address.AnsiPos(city) == 1)
			PrintToMemo(person, Memo1);
		person = person->next;
	}
}
