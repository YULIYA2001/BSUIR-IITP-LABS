...
void __fastcall TForm1::Button1Click(TObject *Sender)
{
	...
	if(Edit9->Text == "0" || Edit10->Text == "0.0")
	{
		...
		ShowMessage("Установите количество мест и требуемый средний балл");
	}
	else if (StrToInt(Edit10->Text) > 10)
	{
		Edit10->Text="";
        Application->Title="ОШИБКА!";
		ShowMessage("Недопустимое значение среднего балла");
    }
	else
	{
		...
		if (StrToInt(Edit5->Text) > 10)		{	Edit5->Text="";		check = false;	}		if (StrToInt(Edit6->Text) > 10)		{	Edit6->Text="";		check = false;	}		if (StrToInt(Edit7->Text) > 10)		{	Edit7->Text="";		check = false;	}		if (!check)		{			Application->Title="ОШИБКА!";
			ShowMessage("Ошибка ввода. Проверьте введенные данные");
			...
			//(добавление n+1 человека, удаление худшего)
			while (personList->GetSize() > StrToInt(Edit9->Text))
			{
				Memo1->Lines->Add("Абитуриент " + personList->DeletePerson()
					+ " был удален в связи с нехваткой мест");
			}
		}	}}
...
//очищение мемо при выборе пункта поиска
...
void __fastcall TForm1::Button4Click(TObject *Sender)
{
	...
	if (Memo1->Lines->Text.IsEmpty())
		Memo1->Lines->Add("Данные не найдены");
}
...
//перескакивание курсора при заполнении Edit-ов
